...
class AuthController {
  ...
  public register = async (req: Request<{}, RegisterUserInput['body']>, res: Response, next: NextFunction) => {
    try {
      ...
      if (user) return void res.status(400).json({ message: 'User already exists' });
      ...
      if (!_token) return void res.status(500).json({ message: 'Token not found' });
      ...
      res.status(201).json({ data: newUser._id, token: _token });
    } catch (err: any) {
  ...
  public login = async (req: Request<{}, LoginUserInput['body']>, res: Response, next: NextFunction) => {
    try {
      ...
      if (!user) return void res.status(400).json({ message: 'User does not exist' });

      // Check if password is correct
      const isMatch = await bcrypt.compare(payload.password, user.password);
      if (!isMatch) return void res.status(400).json({ message: 'Invalid credentials' });
      ...
      // Send response
      return void res.status(200).json({ token: _token, data: { id: user._id, email: user.email } });
    } catch (err: any) {
      res.status(500).send('Internal Server Error');
      console.error(err.message);
    }
  ...
  public verifyEmail = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!token) return void  res.status(400).json({ message: 'Token not found' });

      const user = await this.userService.getUserByToken(token);

      if (!user) return void  res.status(400).json({ message: 'User not found' });

      if (user.verified) return void  res.status(400).json({ message: 'User already verified' });
